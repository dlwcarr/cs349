...
public class CanvasModel {
	...
	public void saveAnimation() {
		...
		j.add("frames", new JsonPrimitive(new Integer(totalFrames)));
		...
		for (Map.Entry<LineSegment, ArrayList<AffineTransform>> entry : transformations.entrySet()) {
			...
			shape.add("points", entry.getKey().convertToJson());

			transforms = new JsonArray();
			for (AffineTransform t : entry.getValue()) {
				if (t == null)
					transforms.add(JsonNull.INSTANCE);
				else {
					transform = new JsonObject();
					transform.add("x", new JsonPrimitive(new Integer((int)t.getTranslateX())));
					transform.add("y", new JsonPrimitive(new Integer((int)t.getTranslateY())));
					transforms.add(transform);
				}
			}

			shape.add("transforms", transforms);
		...
		String filename = formatter.format(new Date()) + ".json";
		File dir = new File("Saved Animations");
		File save = new File(dir, filename);

		try {
			FileWriter fstream = new FileWriter(save);
